...
## may be redudant but who cares
def get_density(chi, temperature):
    c1 = mu*RADA*temperature**3/6/KB
    c2 = 9*KB*chi**2/mu/RADA**2/temperature**7
    return c1*(np.sqrt(1+c2)-1)

## f is the amount to change guess by by every itereation
def get_temperature_from_density(temperature_guess, chi0, density0, f=0.005):
    ## first get a better temperature gues to feed into fsolve
    def density_difference(temperature):
        return get_density(chi0, temperature) - density0

    ## density is a decreasing function of temperature generically (I hope!!)
    A0 = np.zeros(temperature_guess.shape)
    A0 = np.where(density_difference(temperature_guess) > 0, 1, A0)
    A0 = np.where(density_difference(temperature_guess) < 0, -1, A0)
    ...
    while True:
        An = np.where(density_difference(temperature_guess) > 0, 1, An)
        An = np.where(density_difference(temperature_guess) < 0, -1, An)
        An = np.where(np.abs(An - A0) > 0, 0, An)
        # print(np.sum(np.abs(An)))
        temperature_guess *= 1 + f * An
    ...
    temp = fsolve(density_difference, temperature_guess, xtol=1e-6)
    return temp
