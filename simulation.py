...
class Simulation:
    def __init__(self, sigma0, entropy0, params=Params(), verbose=True, progress_message=None):
        ...
        if self.params.RESTART and self.params.SAVE:
            ...
            if self.params.SAVE:
                ...
                old_dat = glob.glob(self.sim_dir+"/*.dat")
                for f in old_dat:
                    os.remove(f)
    ...
    def evolve(self):
        ...
        if self.params.SAVE:
            ...
            sigma_files = sorted(glob.glob(self.sim_dir+"/sigma.*.dat"))
            entropy_files = sorted(glob.glob(self.sim_dir+"/entropy.*.dat"))

            sigma_array, entropy_array, t_array, nn = [], [], [], 0
            if glob.glob(self.sim_dir+"/*.npy") and False: ## this is really annoying!!
                sigma_array = np.load(self.sim_dir+"/sigma.npy")
                entropy_array = np.load(self.sim_dir+"/entropy.npy")
                t_array = np.load(self.sim_dir+"/tsave.npy")
            else:
