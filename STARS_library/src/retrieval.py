...
def retrieval(mass, age, beta, retrieval_input_dir, retrieval_scratch_dir, retrieval_output_dir):
        ...
        for i, (m, t) in enumerate(zip(m_array_temp, t_array_temp)):
            ...
            if (mass == m) & (age == t):
                ...
                if len(sel_beta_temp) != 0:
                    ret_dir = os.path.abspath(retrieval_output_dir + dirs[i] + '/' + beta_files[sel_beta_temp][0])
                    #print('Already retrieved %s' % ret_dir)
                    return ret_dir
    ...
    if len(sel_mass_and_age) != 0:
        ...
        if len(sel_beta) != 0:
            ...
            ret_dir = retrieval_output_dir + outputdirs[sel_mass_and_age][0] + '/' + beta_files[sel_beta][0]
    ...
    # else check if already have this mass
        ...
        if len(sel_beta) != 0:
            ...
            ret_dir = retrieval_output_dir + new_dir + '/' + beta_files[sel_beta][0]
